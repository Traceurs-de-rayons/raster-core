...
bool RasterPipeline::Impl::rebuildPipeline(std::string &errorMessage) {
  ...
  // }
	...
	task->addVertexBuffer(&options.sharedResources->vertexBuffer); // add secu
	task->setIndexBuffer(&options.sharedResources->indexBuffer, VK_INDEX_TYPE_UINT32); // add secu

	task->setIndexedDrawParams(options.sharedResources->indexCount, 1, 0, 0, 0);
