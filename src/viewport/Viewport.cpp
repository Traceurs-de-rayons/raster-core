...
std::unique_ptr<Viewport> Viewport::Impl::create(uint32_t id, const std::string& name,
                                                  uint32_t width, uint32_t height,
                                                  ViewportOutput outputType, SDL_Window* window) {
    return std::unique_ptr<Viewport>(new Viewport(id, name, width, height, outputType, window));
}

// Viewport public interface implementation

Viewport::Viewport(uint32_t id, const std::string& name, uint32_t width, uint32_t height,
                   ViewportOutput outputType, SDL_Window* window)
    : impl_(std::make_unique<Impl>(id, name, width, height, outputType, window))
{
    std::cout << "Viewport '" << impl_->name << "' created (ID: " << impl_->id 
...
void Viewport::updateScene(const Scene& scene) {
    ...
    if (!impl_->pipeline) {
        // Initialize pipeline unconditionally - we need it even for empty scenes
        // to create the framebuffer and image views that ImGui needs
...
void Viewport::render() {
    ...
    // TODO: Apply render mode (wireframe, normals, etc.)
...
const unsigned char* Viewport::getImageData() const {
    ...
    // TODO: Map buffer and return data
...
size_t Viewport::getImageDataSize() const {
    ...
    return static_cast<size_t>(impl_->width) * impl_->height * 4; // RGBA
}
